<?php

namespace App\Enums;

final class PromotionStatus extends AbstractEnum
{
    public const RAW = 0;

    public const PENDING = 1;

    public const APPROVED = 2;

    public const INPROGRESS = 3;

    public const END = 4;

    public static function getName($value)
    {
        switch ($value) {
            case static::RAW:           return __('enum.promotion_status.name.raw');
            case static::PENDING:       return __('enum.promotion_status.name.pending');
            case static::APPROVED:      return __('enum.promotion_status.name.approved');
            case static::INPROGRESS:    return __('enum.promotion_status.name.inprogress');
            case static::END:           return __('enum.promotion_status.name.end');
            default:                    return $value;
        }
    }

    public static function getLabel($value)
    {
        switch ($value) {
            case static::RAW:           return __('enum.promotion_status.label.raw');
            case static::PENDING:       return __('enum.promotion_status.label.pending');
            case static::APPROVED:      return __('enum.promotion_status.label.approved');
            case static::INPROGRESS:    return __('enum.promotion_status.label.inprogress');
            case static::END:           return __('enum.promotion_status.label.end');
            default:                    return $value;
        }
    }

    public static function getIcon($value)
    {
        switch ($value) {
            case static::RAW:           return __('enum.promotion_status.icon.raw');
            case static::PENDING:       return __('enum.promotion_status.icon.pending');
            case static::APPROVED:      return __('enum.promotion_status.icon.approved');
            case static::INPROGRESS:    return __('enum.promotion_status.icon.inprogress');
            case static::END:           return __('enum.promotion_status.icon.end');
            default:                    return $value;
        }
    }

    public static function getNextStatus($value)
    {
        switch ($value) {
            case static::RAW:           return null;
            case static::PENDING:       return static::APPROVED;
            case static::APPROVED:      return static::PENDING;
            case static::INPROGRESS:    return static::END;
            case static::END:           return static::PENDING;
            default:                    return null;
        }
    }

    public static function getAction($value)
    {
        switch ($value) {
            case static::RAW:           return null;
            case static::PENDING:       return __('enum.promotion_status.action.pending');
            case static::APPROVED:      return __('enum.promotion_status.action.approved');
            case static::INPROGRESS:    return __('enum.promotion_status.action.inprogress');
            case static::END:           return __('enum.promotion_status.action.end');
            default:                    return null;
        }
    }

    public static function getActionLabel($value)
    {
        switch ($value) {
            case static::RAW:           return null;
            case static::PENDING:       return __('enum.promotion_status.action_label.pending');
            case static::APPROVED:      return __('enum.promotion_status.action_label.approved');
            case static::INPROGRESS:    return __('enum.promotion_status.action_label.inprogress');
            case static::END:           return __('enum.promotion_status.action_label.end');
            default:                    return null;
        }
    }

    public static function getList(): array
    {
        return [
            static::RAW,
            static::PENDING,
            static::APPROVED,
            static::INPROGRESS,
            static::END
        ];
    }
}