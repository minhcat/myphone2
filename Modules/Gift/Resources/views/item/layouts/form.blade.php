@php
    $gift_product_item = isset($gift_product_item) ? $gift_product_item : new Modules\Sale\Entities\SaleProduct;
@endphp
<div class="row">
    <div class="col-lg-12">
        <div class="box box-primary">
            ...
            <form action="{{ $form['url'] }}" method="{{ $form['method'] == 'GET' ? 'GET' : 'POST' }}">
                ...
                <div class="box-body">
                    <div class="row">
                        <div class="col-lg-6 product-col {{ $gift_product_item->target_type !== TargetType::PRODUCT && $form['title'] === 'Edit' ? 'hidden' : '' }}">
                            <div class="form-group">
                                ...
                                <select id="target_id" class="form-control select-required" aria-placeholder="not select" name="target_id">
                                    ...
                                    @foreach($products as $product)
                                        @if (old('target_type') == TargetType::PRODUCT || $gift_product_item->target_type === TargetType::PRODUCT)
                                        <option value="{{ $product->id }}" {{ old('target_id') == $product->id || $gift_product_item->target_id === $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                                        @else
                        ...
                        <div class="col-lg-6 variant-col {{ $gift_product_item->target_type !== TargetType::VARIANT ? 'hidden' : '' }}">
                            <div class="form-group">
                                ...
                                <select id="target_id2" class="form-control select-required" aria-placeholder="not select" name="target_id">
                                    ...
                                    @foreach($variants as $variant)
                                        @if (old('target_type') == TargetType::VARIANT || $gift_product_item->target_type === TargetType::VARIANT)
                                        <option value="{{ $variant->id }}" {{ old('target_id') == $variant->id || $gift_product_item->target_id === $variant->id ? 'selected' : '' }}>{{ $variant->name }}</option>
                                        @else
                            ...
                            <div class="form-group">
                                ...
                                <select id="target_type" class="form-control" aria-placeholder="not select" name="target_type">
                                    @foreach($target_types as $target_type)
                                        <option value="{{ $target_type->code }}" {{ old('target_type') == $target_type->code || $gift_product_item->target_type === $target_type->code ? 'selected' : '' }}>{{ $target_type->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="form-group">
                                ...
                                <input id="quantity" type="number" class="form-control" name="quantity" value="{{ old('quantity', $gift_product_item->quantity) }}" autocomplete="quantity">
                            </div>
                        </div>
                    </div>
                ...
                <div class="box-footer">
                    <a href="{{ route('admin.gift.product.item.index', ['gift_id' => $gift_id, 'gift_product_id' => $gift_product_id]) }}" class="btn btn-default">Back</a>
