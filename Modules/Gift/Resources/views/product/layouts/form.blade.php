                            ...
                            <div class="form-group">
                                ...
                                <select id="target_id" class="form-control select-required" aria-placeholder="not select" name="target_id">
                                    ...
                                    @foreach($products as $product)
                                        @if (old('target_type') == TargetType::PRODUCT || $gift_product->target_type === TargetType::PRODUCT)
                                        <option value="{{ $product->id }}" {{ old('target_id') == $product->id || $gift_product->target_id === $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                                        @else
                            ...
                            <div class="form-group">
                                ...
                                <select id="target_id2" class="form-control select-required" aria-placeholder="not select" name="target_id">
                                    ...
                                    @foreach($variants as $variant)
                                        @if (old('target_type') == TargetType::VARIANT || $gift_product->target_type === TargetType::VARIANT)
                                        <option value="{{ $variant->id }}" {{ old('target_id') == $variant->id || $gift_product->target_id === $variant->id ? 'selected' : '' }}>{{ $variant->name }}</option>
                                        @else
                            ...
                            <div class="form-group">
                                ...
                                <select id="target_type" class="form-control" aria-placeholder="not select" name="target_type">
                                    @foreach($target_types as $target_type)
                                        <option value="{{ $target_type->code }}" {{ old('target_type') == $target_type->code || $gift_product->target_type === $target_type->code ? 'selected' : '' }}>{{ $target_type->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="form-group">
                                ...
                                <input id="quantity" type="number" class="form-control" name="quantity" value="{{ old('quantity', $gift_product->quantity) }}" autocomplete="quantity">
                            </div>
                        </div>
                    </div>
