        ...
        <div class="box box-primary box-main">
            ...
            <div class="box-body">
                        ...
                        <tbody class="tbody-data hidden">
                            @foreach ($invoices as $key => $invoice)
                                <tr>
                                    ...
                                    @if($invoice->user)
                                    @can('user:read')
                                    <td><a href="{{ route('admin.user.show', $invoice->user->id) }}">{{ $invoice->user->account }}</a></td>
                                    @else
                                    <td>{{ $invoice->user->account }}</td>
                                    @endif
                                    @else
                                    <td></td>
                                    @endif
                ...
                <div class="table-footer mt-3">
                    <div class="row">
                        <div class="col-lg-12">
                            {{ $invoices->appends($_GET)->links(get_admin_theme_extend($admin_active_theme, 'paginate')) }}
                        </div>
                    </div>
                </div>
            </div>
