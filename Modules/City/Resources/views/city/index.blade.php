        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                        ...
                        <tbody>
                            @foreach ($cities as $key => $city)
                                <tr>
                                    ...
                                    @if ($city->user)
                                    @can('user:read')
                                    <td><a href="{{ route('admin.user.show', $city->user->id) }}">{{ $city->user->account }}</a></td>
                                    @else
                                    <td>{{ $city->user->account }}</td>
                                    @endcan
                                    @else
                                    <td></td>
                                    @endif
                ...
                <div class="table-footer mt-3">
                    <div class="row">
                        <div class="col-lg-12">
                            {{ $cities->appends($_GET)->links(get_admin_theme_extend($admin_active_theme, 'paginate')) }}
                        </div>
                    </div>
                </div>
            </div>
