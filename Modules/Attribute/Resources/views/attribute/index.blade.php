        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                        ...
                        <tbody>
                            @foreach ($attributes as $key => $attribute)
                                <tr>
                                    ...
                                    @if ($attribute->user)
                                    @can('user:read')
                                    <td><a href="{{ route('admin.user.show', $attribute->user->id) }}">{{ $attribute->user->account }}</a></td>
                                    @else
                                    <td>{{ $attribute->user->account }}</td>
                                    @endcan
                                    @else
                                    <td></td>
                                    @endif
                ...
                <div class="table-footer mt-3">
                    <div class="row">
                        <div class="col-lg-12">
                            {{ $attributes->appends($_GET)->links(get_admin_theme_extend($admin_active_theme, 'paginate')) }}
                        </div>
                    </div>
                </div>
            </div>
